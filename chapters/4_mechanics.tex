...
\chapter{Mechanik}
	...
	\section{Transmission}\label{sec:transmission}
		...
		\begin{figure}[h]
			\centering
			\includesvg[width=\textwidth, inkscapelatex=false]{Calc/T_v_vs_zetas}
		...
		%
		Um das Drehmoment von der Motorwelle auf die Rolle zu übertragen, wird ein Riemensystem mit \textit{High Torque Drive}-Profil (HTD\nomenclature[A]{HTD}{High Torque Drive}) in Zahnteilung 5M (\qty{5}{mm} Abstand zwischen zwei Zähnen entlang der neutralen Faser) gewählt. 
		...
		\begin{figure}[h]
			...
			\label{fig:kegels}
		\end{figure}
		Unter Zuhilfenahme einer Fühlerlehre, ermitteln der Maße und mehreren Iterationen im FDM-Verfahren\nomenclature[A]{FDM}{Fused Deposition Modeling} 3D-gedruckter Modelle konnten die inneren Profile der Rollen in ausreichender Genauigkeit so in ein CAD-Modell übertragen werden, dass daraus eine Passung für die getriebeseitige Zahnscheibe modelliert werden kann.
		\begin{figure}[h]
			\centering
